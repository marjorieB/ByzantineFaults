...
void add_new_worker(const char * name, char * myMailbox, int id) {
	...
	ack = MSG_task_create("ack", 0, (strlen("ack") + strlen(myMailbox)) * sizeof(char), myMailbox);
...
void treat_tasks(xbt_dynar_t * w, msg_task_t * task_to_treat, int id) {
	...
	t->nb_replication = 0;
	//t->res = (struct p_answer_worker *)malloc(sizeof(struct p_answer_worker));
	...
	//printf("primary-%d: treat_tasks %s %s\n", id, t->client, t->task_name);
	nb++;
	struct p_worker p_w;
	unsigned int cpt;

	xbt_dynar_foreach (*w, cpt, p_w) {
		//printf("primary-%d: send task %s %s to %s\n", id, t->client, t->task_name, p_w.mailbox);  
...
void try_to_treat_tasks(char * mailbox, int id) {
	...
	//printf("primary-%d: in try_to_treat_tasks: nb_workers=%ld, nb_tasks=%d\n", id, xbt_dynar_length(workers[id]), xbt_fifo_size(tasks[id]));
		...
		for(i = xbt_fifo_get_first_item(tasks[id]); ((i) ? (task_to_treat = (msg_task_t *)(xbt_fifo_get_item_content(i))) : (NULL)); i = xbt_fifo_get_next_item(i)) {
			...
			if (xbt_dynar_length(workers[id]) >= group_formation_min_number) {
				if (group_formation_strategy == FIXED_FIT) {
					...
					if (simulator == ARANTES) {
						//printf("go to execute formGroup_random_fit_arantes(...)\n");
	...
	if (centrality == DISTRIBUTED) {
		if((xbt_fifo_size(active_groups[id]) == 0) && (xbt_fifo_size(tasks[id]) != 0) && (xbt_fifo_size(processing_tasks[id]) == 0)) {
			// we have tried to form group but we have'nt been able to do so
			if (able_to_send_fusion[id] == 1) {
				doing_fusion[id] = 1;
				//printf("primary-%d: sending FUSION\n", id);
...
double valueCond2 (struct p_answer_worker * res, struct p_task * p_t) {
	...
	//printf("START VALUECOND2, number of groups that executed the task %ld\n", xbt_dynar_length(p_t->w_answers));
	...
	xbt_dynar_foreach(p_t->w_answers, cpt, p) {
		...
		if (res == xbt_dynar_get_ptr(p_t->w_answers, cpt)) {
			xbt_dynar_foreach(p.worker_reputations, nb, reputation) {
				//printf("part if : worker belonging to the group is: with reputation %d\n", reputation);
				if (reputation == 100) {
					Psc = Psc * 0.999;
					tmp = tmp * (1.0 - 0.999);
				}
				...
				//printf("part if : en cours value of Psc = %f, value of tmp = %f\n", Psc, tmp);
			}
			complexity = complexity + 7.0 * nb;
		}
		else {
			xbt_dynar_foreach(p.worker_reputations, nb, reputation) {
				//printf("part else : worker belonging to the group is: with reputation %d\n", reputation);
				if (reputation == 100) {
					Psc = Psc * (1.0 - 0.999);
				}
				else {
				...
				//printf("part else : en cours value of Psc = %f, value of tmp = %f\n", Psc, tmp);
			} 
	...
	//printf("value of Psc = %f, value of tmp = %f\n", Psc, tmp);
...
void compute_majoritary_answer(struct p_task * p_t, int * nb_majoritary_answer, double * max_value) {
	...
	xbt_dynar_foreach(p_t->w_answers, cpt, w_a) {
		if ((simulator == SONNEK) || ((simulator == ARANTES) && (group_formation_strategy == FIXED_FIT))) {
			...
			//printf("go to search wich is the good answer, number of workers %d\n", p_t->nb_forwarded);
...
void writes_data (char * client_name, char * task_name, double time_start_task, char fail, unsigned int long answer, long int number_workers_used, double targetLOC, int id) {
	...
	int to_write = 0;
	 ...
	 for(i = xbt_fifo_get_first_item(active_groups[id]); ((i) ? (n = (xbt_dynar_t *)(xbt_fifo_get_item_content(i))) : (NULL)); i = xbt_fifo_get_next_item(i)) {
		nb++;
		//printf("dans write group number %d\n", nb);
	...
	if (nb_answers_written_data_csv >= 800) {
...
void send_answer_Sonnek(struct p_task * n, int nb_majoritary_answer, char * process, int id) {
	...
	if ((nb_majoritary_answer == 1) && (xbt_dynar_length(n->res->worker_names) > floor((double)n->nb_forwarded/2.0))) {
		...
		msg_task_t answer_client = MSG_task_create("answer", 0, sizeof(char) * (strlen("answer") + strlen(n->task_name)), n->task_name);
		...
		msg_task_t fail = MSG_task_create("fail", 0, sizeof(char) * (strlen("fail") + strlen(n->task_name)), n->task_name);
...
void send_answer_Arantes(struct p_task * n, int nb_majoritary_answer, char * process, char * stop_replication, double max_value, int id) {
	double complexity = 0.0;
	//printf("dans send_answer_arantes\n");



	//printf("nb_majoritary_answer = %d\n", nb_majoritary_answer);
	//printf("n->nb_forwarded=%d\n", n->nb_forwarded);
	
	//printf("max_value=%f\n", max_value);
	
	//printf("1.0 - n->targetLOC = %f\n", 1.0 - n->targetLOC);
	//printf("xbt_dynar_length(n->res->worker_names)=%ld\n", xbt_dynar_length(n->res->worker_names));
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if (*stop_replication == 1) {
		...
		if ((nb_majoritary_answer == 1) && (xbt_dynar_length(n->res->worker_names) > 1)) {
			if (nb_workers_for_stationary >= stationary_regime) {
				//printf("final answer = %ld\n", n->res->answer);
				writes_data(n->client, n->task_name, n->start, -1, n->res->answer, n->nb_forwarded, n->targetLOC, id);//xbt_dynar_length(n->res->worker_names), id);
			}	
			//printf("go to send the answer\n");		
			// send the answer to the client
			msg_task_t answer_client = MSG_task_create("answer", 0, sizeof(char) * (strlen("answer") + strlen(n->task_name)), n->task_name);
			MSG_comm_destroy(MSG_task_isend(answer_client, n->client));
		}
		else {
			//printf("there isn't any good answer -> failed\n");
			n->final_answer = n->res->answer;
			if (nb_workers_for_stationary >= stationary_regime) {
				writes_data(n->client, n->task_name, n->start, 1, n->res->answer, n->nb_forwarded, n->targetLOC, id);//xbt_dynar_length(n->res->worker_names), id);
			}	
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//printf("fin send_answer_arantes\n");
	msg_task_t task_complexity = MSG_task_create("task_complexity", complexity + 1.0, 0, NULL);
	MSG_task_execute(task_complexity);
	MSG_task_destroy(task_complexity);
}
		...
		if ((strcmp(n->client, w_t->client) == 0) && (strcmp(n->task_name, w_t->task_name) == 0)) {
			...
			if (((n->nb_answers_received + n->nb_crashed) == n->nb_forwarded) && (inAdditional_replication_tasks(n, id) == -1)) {
				...
				if (simulator == SONNEK) {
					...
					//printf("end of the send_answer_arantes\n");
				}
				//free(n->w_answers);
				complexity += 6.0;
			}
...
void send_change(int id) {
	...
	*toSend = xbt_dynar_new(sizeof(struct p_worker), NULL); 
	...
	//printf("primary-%d: sending the change message to the first-primary\n",id);
...
void try_load_balancing_overload(int id, char * mailbox) {
	...
	/*//printf("%s, in try_load_balancing_overload\n", mailbox);
	//printf("xbt_dynar_length(workers[id])=%ld\n", xbt_dynar_length(workers[id]));
	//printf(" xbt_fifo_size(active_groups[id])=%d\n",  xbt_fifo_size(active_groups[id]));
	//printf("number_workers_too_high = %d\n", number_workers_too_high);
	//printf("floor((number_workers_too_high / 2)) + 1)) = %d\n", number_workers_too_high / 2 + 1);*/



	if (able_to_send_division[id] == 1) {
		//printf("I am able to divide myself\n");
		if (((xbt_dynar_length(workers[id]) + xbt_fifo_size(active_groups[id])) >= number_workers_too_high) && (xbt_dynar_length(workers[id]) >= (floor(number_workers_too_high / 2) + 1))) {
			if (distributed_strategies == RANDOM) {	
				//printf("go to execute try_load_balancing_random_overload\n");			
...
void forward_to_first_primary(msg_task_t task) {
	struct clientDataTask * data_task;
	char * data_join;
	xbt_dynar_t * data_change;	

	//printf("I have to forward the task %s\n", MSG_task_get_name(task));

	if (!strncmp(MSG_task_get_name(task), "task", strlen("task"))) {
		data_task = (struct clientDataTask *) malloc(sizeof(struct clientDataTask));
		data_task = (struct clientDataTask *) MSG_task_get_data(task);
		MSG_comm_destroy(MSG_task_isend(MSG_task_create(MSG_task_get_name(task), 0, MSG_task_get_data_size(task), data_task), first_primary_name)); 
	}
	else if (!strcmp(MSG_task_get_name(task), "join")) {
		data_join = (char *) malloc(sizeof(char) * MAILBOX_SIZE);
		strcpy(data_join, (char *) MSG_task_get_data(task));
		MSG_comm_destroy(MSG_task_isend(MSG_task_create(MSG_task_get_name(task), 0, MSG_task_get_data_size(task), data_join), first_primary_name)); 
	}
	else {
		struct p_worker * worker = (struct p_worker *) malloc(sizeof(struct p_worker));
		worker = (struct p_worker *) MSG_task_get_data(task);
		
		data_change = (xbt_dynar_t *) malloc(sizeof(xbt_dynar_t));
		*data_change = xbt_dynar_new(sizeof(struct p_worker), NULL);

		xbt_dynar_push(*data_change, worker);

		////printf("in forward, we forward a change: value of the dynar sent %ld\n", xbt_dynar_length(*data_change));

		/*int i;
		int until = xbt_dynar_length(rec);

		for(i = 0; i < until; i++) {
			strcut p_worker *w = (struct p_worker *) malloc(sizeof(struct p_worker));
		...
		MSG_comm_destroy(MSG_task_isend(MSG_task_create(MSG_task_get_name(task), 0, MSG_task_get_data_size(task), data_change), first_primary_name)); 
	}
...
void execute_fusion (int id) {
	...
	toSend->tasks = xbt_fifo_new();
	//toSend->processing_tasks = xbt_fifo_new();


	int until = xbt_dynar_length(workers[id]);
	double complexity = 1.0;
	int until_tasks = xbt_fifo_size(tasks[id]);
	...
	for (i = 0; i < until_tasks; i++) {
		xbt_fifo_push(toSend->tasks, xbt_fifo_pop(tasks[id]));
	}
	...
	complexity = complexity + until * 2.0 + until_tasks + 1.0;
...
void update_limit_max (msg_task_t task, int id) {
	char * new_limit = (char *)malloc(sizeof(char));
	new_limit = (char *)MSG_task_get_data(task);

	//printf("IN UPDATE_LIMIT_MAX MY OLD MAX_REPUTATION IS: %d\n", reputations_primary[id].max_reputation);
...
int primary (int argc, char * argv[]) {
	...
	if (centrality == DISTRIBUTED) {
		if (id == 0) {
			...
			//printf("value of first_primary_name %s\n", first_primary_name);
	...
	while (1) {
		...
		if (!strcmp(MSG_task_get_name(task_todo), "finalize")) {
			...
			//printf("AFTER UPDATE_LIMIT_MAX MY NEW MAX_REPUTATION IS: %d\n", reputations_primary[id].max_reputation);
		}
