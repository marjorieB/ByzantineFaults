...
double compute_LOC(xbt_dynar_t * w) {
	...
	printf("valeur de m %d, taille de w %ld\n", m, xbt_dynar_length(*w));
...
// this function is executed by a primary. it will permit to form groups with workers sorted by reputation until the whole group obtain a reputation above a certain treshlod
void formGroup_first_fit() {
	...
	while (xbt_dynar_length(workers) >= group_formation_min_number) {
		...
		*w = xbt_dynar_new(sizeof(struct p_worker), NULL);	
		...
		do {
			...
			printf("iteration %ld value of the LOC = %f\n", xbt_dynar_length(*w), LOC);

		} while(((LOC < group_formation_target_value) || (xbt_dynar_length(*w) < group_formation_min_number)) && (xbt_dynar_length(*w) != group_formation_max_number) && (xbt_dynar_length(workers) != 0));
		printf("I am out, and I have %ld workers on my group\n", xbt_dynar_length(*w));
		if (xbt_dynar_length(workers) == 0) {
			if(((LOC >= group_formation_target_value) && (xbt_dynar_length(*w) >= group_formation_min_number)) || (xbt_dynar_length(*w) == group_formation_max_number)) {
				xbt_fifo_push(inactive_groups, w);
				break;
			}
			else {
				...
				printf("I have to put the workers back in the worker list\n");
				for (i = 0; i < until; i++) {
...
void formGroup_first_fit_Sonnek() {
	...
	while (xbt_dynar_length(workers) >= group_formation_min_number) {
		...
		*w = xbt_dynar_new(sizeof(struct p_worker), NULL);	
		...
		} while(((LOC < group_formation_target_value) || (xbt_dynar_length(*w) < group_formation_min_number) || (xbt_dynar_length(workers) % 2 == 0)) && (xbt_dynar_length(*w) != group_formation_max_number) && (xbt_dynar_length(workers) != 0));
		printf("I am out, and I have %ld workers on my group\n", xbt_dynar_length(*w));
		if (xbt_dynar_length(workers) == 0) {
			if(((LOC >= group_formation_target_value) && (xbt_dynar_length(*w) >= group_formation_min_number) && (xbt_dynar_length(workers) % 2 != 0)) || ((xbt_dynar_length(*w) == group_formation_max_number) && (xbt_dynar_length(workers) % 2 != 0))) {
				xbt_fifo_push(inactive_groups, w);
				break;
			}
			else {
	...
	if (index <= FIRST_ITEM + 1) {
		...
		*LOC = compute_LOC(w);
		if (*LOC >= group_formation_target_value) {
			...
			xbt_dynar_pop((xbt_dynar_t const)w, toAdd);
	...
	while (xbt_dynar_length(workers) >= group_formation_min_number) {
		...
		*w = xbt_dynar_new(sizeof(struct p_worker), NULL);	
		...
		do {
			...
			printf("iteration %ld value of the threshold = %f\n", xbt_dynar_length(*w), res);
		} while(((res > group_formation_target_value) || (xbt_dynar_length(*w) < group_formation_min_number)) && (xbt_dynar_length(*w) != group_formation_max_number) && (xbt_dynar_length(workers) != 0));
		if (xbt_dynar_length(workers) == 0) {
			if(((res <= group_formation_target_value) && (xbt_dynar_length(*w) >= group_formation_min_number)) || (xbt_dynar_length(*w) == group_formation_max_number)) {
				xbt_fifo_push(inactive_groups, w);
